...
export default function MatchResultPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("http://localhost:5000/api/matches/result/batch", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(serverMessage || "Failed to process match result data.");
      }
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
        <section className="rounded-[2rem] bg-white p-8 shadow-[0_25px_80px_rgba(15,23,42,0.08)]">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-slate-500">Match results</p>
              <h1 className="mt-3 text-3xl font-semibold text-slate-900">Paste match result JSON and submit it</h1>
            </div>
            <p className="max-w-xl text-sm text-slate-600">
              Enter your match result JSON below to send it to the backend and view the processed response.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-6">
            <div className="flex flex-col w-full max-w-full overflow-hidden">
              ...
              <textarea
                ...
                placeholder="Paste your match result JSON here..."
              />
            </div>
...
mongodb+srv://codepappi:KUPj3QyIJ09FRV3u@cluster0.5ajpmas.mongodb.net/?appName=Cluster0
DB NAME IKUWE crm-main
